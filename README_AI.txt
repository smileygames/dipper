...
・対応サービス追加は歓迎だが、実機検証できる人が責任を持って行う（Issueで分離）
...
・コミットメッセージは日本語で記述する
・後から何を変更したか分かる内容にする
・説明は判りやすく簡潔に。重要なのは変更内容を記録として残すこと
・Issue 番号は文末に付与する
...
・外部コマンド（dig など）はハングする可能性がある
・パイプラインの終了コードや timeout 未指定は、
  実運用でシステム全体を止める原因になり得る
・テストは「エラーを出すか」ではなく
  「何もしないで通り過ぎる挙動」も正として記録する
・特に外部ネットワークに依存する処理は、
  「正しく失敗する」より「静かに何もしない」方が安全な場合がある
...
・bin 配下の各スクリプトは、設定値が外部から注入される前提で設計されている。
